...
namespace API
{
    public class EmotionAnalysisManager
    {
        ...
        public Dictionary<int, double> GetAverageSentiment(int userId, int duration, string timeUnit)
        {
            ...
            {
                result[i] = groupedSentiments.ContainsKey(i) ? groupedSentiments[i] - 1 : 0.0;
            }

            return result;
        }

        // 清除当前用户的 EmotionRecord 数据
        public async Task ClearEmotionRecordsAsync(int userId)
        {
            var records = _dbContext.EmotionRecords.Where(record => record.UserId == userId);
            _dbContext.EmotionRecords.RemoveRange(records);
            await _dbContext.SaveChangesAsync();
        }

        public async Task ClearKeywordFrequenciesAsync(int userId)
        {
            var frequencies = _dbContext.KeywordFrequencies.Where(keyword => keyword.UserId == userId);
            _dbContext.KeywordFrequencies.RemoveRange(frequencies);
            await _dbContext.SaveChangesAsync();
        }
    }
}
